...
// c in 32-bit, size of pointer varialbe is 4byte
...
int mm_init(void)
{
    ...
    PUT(heap_listp+(2*WSIZE), NULL);                // prev free p
    PUT(heap_listp+(3*WSIZE), NULL);                // next free p
    PUT(heap_listp+(6*WSIZE), PACK(MINIMUM,1));     // prologue footer
